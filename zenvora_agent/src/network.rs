...
fn schedule_screen_capture(
    ...
    force_keyframe: bool,
    webrtc: Option<Arc<crate::webrtc_session::WebRtcSessionManager>>,
) {
    if screen_busy
    ...
    tokio::spawn(async move {
        ...
        match outcome {
            Ok(StreamOutcome::Frame(jpeg)) => {
                if let Some(rtc) = webrtc.as_ref() {
                    let rtc_clone = Arc::clone(rtc);
                    let jpeg_clone = jpeg.clone();
                    tokio::spawn(async move {
                        let _ = rtc_clone.send_media_frame(&jpeg_clone).await;
                    });
                }
    ...
    loop {
        ...
        match connect_result {
            Ok(Ok((ws_stream, _))) => {
                ...
                let (webrtc_sig_tx, mut webrtc_sig_rx) = mpsc::channel::<serde_json::Value>(32);
                let webrtc_mgr = Arc::new(crate::webrtc_session::WebRtcSessionManager::new(webrtc_sig_tx));
                ...
                loop {
                    ...
                    tokio::select! {
                        ...
                        Some(sig_val) = webrtc_sig_rx.recv() => {
                            let _ = write_tx.send(Message::Text(sig_val.to_string()));
                        }
                                    ...
                                    match serde_json::from_str::<IncomingPacket>(&text) {
                                        Ok(packet) => {
                                            ...
                                            if action == "WEBRTC_SIGNAL" {
                                                let mgr = Arc::clone(&webrtc_mgr);
                                                let sig_payload = payload.get("signal").unwrap_or(&payload).clone();
                                                tokio::spawn(async move {
                                                    mgr.handle_signal(&sig_payload).await;
                                                });
                                            }
