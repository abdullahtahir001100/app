...
import { WebRtcClient, WebRtcSignal, WebRtcState } from "@/lib/webrtc-client";
...
type UseScreenRemoteOptions = {
  ...
  /** Optional signal sender for WebRTC P2P */
...
export function useScreenRemote({ subscribe, selectedDeviceRef, mediaDeviceId, sendSignal }: UseScreenRemoteOptions) {
  ...
  useEffect(() => {
    return subscribe((event) => {
      ...
      if (packet.type === "webrtc_signal") {
        const signal = (packet.signal || packet.payload) as WebRtcSignal | undefined;
        if (signal) {
          void webrtcRef.current?.handleRemoteSignal(signal);
        }
        return;
      }
  ...
  useEffect(() => {
    ...
    const unsubFrame = rtc.onFrame((arrBuf) => {
      ...
      void paintFrameRef.current(blob);
    });

    void rtc.start(mediaDeviceId, "screen", sendSignal);
  ...
  }, [mediaDeviceId, sendSignal]);

  const sendUdpControl = useCallback((action: string, payload: Record<string, unknown>): boolean => {
    return webrtcRef.current?.sendControl(action, payload) ?? false;
  }, []);
  ...
  return {
    ...
    sendUdpControl,
    webrtcState,
    isUdpConnected: webrtcState === "connected",
  };
}
