...
export default function CameraPage() {
  ...
  useEffect(() => {
    ...
    void rtc.start(deviceId, "camera", (signal) => {
      gatewayDispatch("WEBRTC_SIGNAL", { signal, stream_type: "camera" }, deviceId);
    });
    ...
    return () => {
      ...
      unsubFrame();
      rtc.close();
      if (webrtcClientRef.current === rtc) {
        webrtcClientRef.current = null;
      }
      setWebrtcState("closed");
    };
  }, [selectedDevice, isCameraOn, gatewayDispatch]);
  ...
  useEffect(() => {
    return subscribe((event) => {
      ...
      if (data.type === "webrtc_signal") {
        const signal = (data.signal || data.payload) as WebRtcSignal | undefined;
        if (signal) {
          void webrtcClientRef.current?.handleRemoteSignal(signal);
        }
        return;
      }
