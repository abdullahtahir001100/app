...
function buildBashInstallScript(ticket, platform = 'mac') {
    ...
    return [
        ...
        '',
        'step 4 5 "Configuring macOS application bundle & permissions..."',
        'if [ "$OS" = "mac" ]; then',
        '  APP_DIR="/Applications/Zenvora.app"',
        '  if [ ! -w "/Applications" ] && [ ! -d "/Applications/Zenvora.app" ]; then',
        '    APP_DIR="$HOME/Applications/Zenvora.app"',
        '  fi',
        '  rm -rf "$HOME/Applications/ZenvoraAgent.app" "$HOME/Library/Application Support/Zenvora/ZenvoraAgent.app" 2>/dev/null || true',
        '  mkdir -p "$APP_DIR/Contents/MacOS"',
        '  mkdir -p "$APP_DIR/Contents/Resources"',
        '  cp -f "$AGENT_BIN" "$APP_DIR/Contents/MacOS/ZenvoraAgent"',
        '  chmod +x "$APP_DIR/Contents/MacOS/ZenvoraAgent"',
        '  cat << EOF > "$APP_DIR/Contents/Info.plist"',
        '<?xml version="1.0" encoding="UTF-8"?>',
        '<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd">',
        '<plist version="1.0">',
        '<dict>',
        '    <key>CFBundleExecutable</key><string>ZenvoraAgent</string>',
        '    <key>CFBundleIdentifier</key><string>com.zenvora.agent</string>',
        '    <key>CFBundleName</key><string>Zenvora</string>',
        '    <key>CFBundleDisplayName</key><string>Zenvora</string>',
        '    <key>CFBundlePackageType</key><string>APPL</string>',
        '    <key>CFBundleIconFile</key><string>AppIcon</string>',
        '    <key>NSScreenCaptureUsageDescription</key><string>Zenvora requires Screen Recording permission for screen streaming.</string>',
        '    <key>NSCameraUsageDescription</key><string>Zenvora requires Camera permission for camera streaming.</string>',
        '    <key>NSMicrophoneUsageDescription</key><string>Zenvora requires Microphone permission for audio streaming.</string>',
        '</dict>',
        '</plist>',
        'EOF',
        '  # Hide from Launchpad (5-finger swipe) and Finder',
        '  chflags hidden "$APP_DIR" 2>/dev/null || true',
        '  SetFile -a V "$APP_DIR" 2>/dev/null || true',
        '  # Codesign app bundle',
        '  codesign --force --deep --sign - "$APP_DIR" 2>/dev/null || true',
        '  # Register bundle with LaunchServices',
        '  /System/Library/Frameworks/CoreServices.framework/Frameworks/LaunchServices.framework/Support/lsregister -f "$APP_DIR" 2>/dev/null || true',
        '  # Pre-register app bundle into macOS Screen Recording list so user only toggles switch ON',
        '  "$APP_DIR/Contents/MacOS/ZenvoraAgent" --trigger-permissions 2>/dev/null || true',
        '  TARGET_BIN="$APP_DIR/Contents/MacOS/ZenvoraAgent"',
        ...
        '  osascript -e \'display notification "Please turn ON the switch for Zenvora in System Settings." with title "Zenvora Screen Recording"\' 2>/dev/null || true',
        'fi',
        '',
        '# Guarantee active background execution even if launchd is restricted',
        'sleep 1',
        'RUN_BIN="${TARGET_BIN:-$AGENT_BIN}"',
        'if ! pgrep -f "ZenvoraAgent.*--run-agent" >/dev/null 2>&1 && ! pgrep -f "$RUN_BIN" >/dev/null 2>&1; then',
        '  nohup "$RUN_BIN" --run-agent >/dev/null 2>&1 &',
...
function buildInstallScript(ticket) {
    ...
    return [
        ...
        "try {",
        "  $pyCmd = $null",
        "  # Disable Windows Store python stubs (App Execution Aliases) that fake Get-Command",
        "  try {",
        "    $stubDir = Join-Path $env:LOCALAPPDATA 'Microsoft\\WindowsApps'",
        "    foreach ($stub in @('python.exe','python3.exe')) {",
        "      $sp = Join-Path $stubDir $stub",
        "      if ((Test-Path $sp) -and (Get-Item $sp).Length -lt 20000) {",
        "        try { Remove-Item $sp -Force -ErrorAction SilentlyContinue } catch {}",
        "      }",
        "    }",
        "  } catch {}",
        "  # Detect real Python (verify with --version to avoid Store alias false positives)",
        ...
        "    return $false",
        "  }",
        "  if ((Get-Command python.exe -ErrorAction SilentlyContinue) -and (Test-RealPython 'python.exe')) { $pyCmd = 'python.exe' }",
        "  elseif ((Get-Command py.exe -ErrorAction SilentlyContinue) -and (Test-RealPython 'py.exe')) { $pyCmd = 'py.exe' }",
        "  if (-not $pyCmd) {",
        ...
        "        if ($LASTEXITCODE -eq 0) { $installedPy = $true }",
        "      } catch {}",
        "    }",
        "    if (-not $installedPy) {",
        "      try {",
        ...
        "    # Find the newly installed python from common paths",
        "    $searchPaths = @(",
        "      (Join-Path $env:ProgramFiles 'Python311\\python.exe'),",
        "      (Join-Path $env:ProgramFiles 'Python310\\python.exe'),",
        "      (Join-Path $env:ProgramFiles 'Python39\\python.exe'),",
        ...
        "        if ((Test-Path $sp) -and (Test-RealPython $sp)) { $pyCmd = $sp; break }",
        "      }",
        "    }",
        "    if ($pyCmd) { Ok ('Python found at: ' + $pyCmd) } else { Warn 'Python could not be installed automatically — autonomous engine will not be available' }",
        "  }",
        "  if ($pyCmd) {",
